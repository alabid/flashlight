...
############## Global Variable ##############
USERNAME = 'vineelkpratap' # os.environ.get('USER')
...
# 3. Your job
/usr/mpi/gcc/openmpi-4.0.4rc3/bin/mpirun {job_script}
...
def main(binary, mode, config, model_path, extra, tag, partition, ngpu, gpu16, cpus_per_task, mem_per_gpu, hours, local):
    ...
    if mode == 'train' or mode == 'fork':
        ...
        checkpoint_path = os.path.join(sub_log_dir, '001_model_last.bin')
    ...
    # Continue mode
        ...
        checkpoint_path = os.path.join(
            log_dir, exp_id, '001_model_last.bin')
    ...
    slrum_params.update({
        ...
        'output': f'{log_dir}/{exp_id}/%j.out',
        'error': f'{log_dir}/{exp_id}/%j.err',
    })
